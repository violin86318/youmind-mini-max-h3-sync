### Full-Reference: 角色登场模板（参数化）

> 🔥 X 社区精选 — 1502♥ 成片案例，提示词与视频均来自[原推](https://x.com/aimikoda/status/2086412223061135392)

#### 📖 描述

可复用的参数化角色登场模板：把 @[char ref] 换成你的角色参考图即可。15 秒四段式节奏——细节特写上摇→背面环绕揭示→标志性小动作特写→全身英雄镜头收尾，附视觉方向/运镜/环境三个锁定模块。
...
- **描述:** 可复用的参数化角色登场模板：把 @[char ref] 换成你的角色参考图即可。15 秒四段式节奏——细节特写上摇→背面…
- **类型:** Full-Reference
- **来源:** [X @aimikoda](https://x.com/aimikoda/status/2086412223061135392)
- **核心技巧:** @[char ref] 参数化引用 + 时间段式分镜（0–4/4–8/8–12/12–15s）+ 细节→局部→面部→全身的递进揭示
- **标签:** 角色登场, 参数化模板, 电影感, 英雄镜头, 可复用
